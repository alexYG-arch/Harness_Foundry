...
# Compatibility exports; the generic implementation no longer imports this module.
from .revision_store import (
    RevisionControlEventStore, REVISION_CONTROL_EVENT_STORE_TABLE_COLUMNS,
    CONTROL_EVENT_STORE_TRIGGER_SQL,
)
...
class ControlEventStore:
    ...
    def __new__(cls, database_path, *, read_only=False, storage_format="LEGACY_HASH_V1"):
        # Preserve the historical construction API, with one implementation of
        # revision storage shared by both the old and generic entrypoints.
        if cls is ControlEventStore and storage_format == "REVISION_V1":
            return RevisionControlEventStore(database_path, read_only=read_only)
    ...
    def __init__(
        ...
        if storage_format != "LEGACY_HASH_V1":
    ...
    @staticmethod
    def _require_exact_existing_schema(connection, columns=None):
        RevisionControlEventStore._require_exact_existing_schema(
            connection, CONTROL_EVENT_STORE_TABLE_COLUMNS if columns is None else columns)
    ...
    def _table_columns(self):
        return CONTROL_EVENT_STORE_TABLE_COLUMNS
