...
def _check_project_command_mode_bindings(
    root: Path,
) -> list[dict[str, Any]]:
    findings: list[dict[str, Any]] = []
    for index_path in sorted(
        root.glob("project_start_packages/*/WORKPACK_INDEX.json")
    ):
        project_root = index_path.parent
        index = legacy._read_json(index_path, findings)
        manifest = legacy._read_json(
            project_root / "COMMAND_MANIFEST.json", findings
        )
        ...
        for workpack in index.get("workpacks", []):
            ...
            execution_mode = workpack.get("execution_mode")
            command_ids = workpack.get("command_ids")
            if (
                not workpack_id
            ...
            for command_id in command_ids:
                ...
                allowed_modes = (
                    command.get("allowed_modes")
                    if isinstance(command, dict)
                    else None
                )
                if (
                    not isinstance(allowed_modes, list)
                    or execution_mode not in allowed_modes
                ):
                    findings.append(
                        _finding(
                            "PROJECT_COMMAND_MODE_CONTRACT_MISMATCH",
                            (
                                f"{project_root.name}:{workpack_id}:"
                                f"{command_id} does not allow {execution_mode}"
                            ),
                        )
                    )
    return findings
...
CHECKS: tuple[tuple[str, CheckFunction], ...] = (
    ...
    ("PROJECT_COMMAND_MODE_BINDINGS", _check_project_command_mode_bindings),
