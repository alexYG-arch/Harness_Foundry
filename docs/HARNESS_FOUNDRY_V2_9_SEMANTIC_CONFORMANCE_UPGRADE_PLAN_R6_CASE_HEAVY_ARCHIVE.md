# Harness Foundry v2.9 R6 案例偏重提案历史快照

> 状态：`HISTORICAL_NON_NORMATIVE_ARCHIVE`
>
> 本文件是 R7 物理拆分前的完整 R6 快照，仅用于回溯修订历史、案例来源和被否定的设计方式。不得作为 v2.9 Normative Core、实现输入、Target Profile 默认值、运行时路由或认证依据。现行提案入口为 `HARNESS_FOUNDRY_V2_9_SEMANTIC_CONFORMANCE_UPGRADE_PLAN.md`。

---
# Harness Foundry v2.9 语义合规升级方案
...
| 当前提案修订 | `PROPOSAL-R6` |
| 触发案例 | V-SCDSL Video Prompt Harness |
| 目标 | 使 Foundry 不仅能证明需求、制品、接口和安装链一致，还能阻止缺少真实运行与独立语义证据的实现被错误认证 |
| 通用性边界 | V-SCDSL 仅作为根因发现案例和首个 Target Profile；平台 Schema、门禁、状态机、CLI、迁移和发布规则不得依赖 V-SCDSL、Prompt、Codex、Seedance、Genome、固定数量、固定 Lane、本机绝对路径、同级私有仓库或未声明的插件 |
...
本次 V-SCDSL 项目表明，这些机制能够证明“正确的流程处理了同一个制品”，却不能天然证明“实现行为满足语义要求”。如果真实语义要求被实现为模板轮转，验证器又只检查数量、Schema、自报指标和 Hash，那么整个控制链仍可能形成形式正确、行为错误的绿色结果。
...
任何平台级规则若直接要求 Codex、Prompt、Genome、Seedance、30 条或固定 Lane 数，均视为 `PLATFORM_DOMAIN_LITERAL_HARDCODED`，不得进入 v2.9 Release。

但“声明案例不是平台规则”还不够。R5 中 V-SCDSL 同时承担触发案例、根因证据、Schema/Oracle 示例、Golden Canary、验收样本和候选生产拓扑，叙事权重仍然过高，可能让设计者或模型围绕首个案例扩展 Gate、Stage、Subharness、重试与转向策略。R6 因此增加案例去偏置边界：
...
实际 v2.9 规范包必须能够在不加载 V-SCDSL Profile、案例正文和 Fixture 的条件下编译、测试并解释完整 Core。案例可以发现问题，但不能单独决定平台向哪里升级。
...
| 能力 | v2.8 规范状态 | 本次案例的物化状态 | v2.9 增量 |
|---|---|---|---|
...
## 3. 本次 V-SCDSL 项目暴露的问题（`NON_NORMATIVE_CASE_EVIDENCE`）
...
### 3.3 为什么 Start Package、校验和 Linkage 仍没有阻止问题
...
```text
目标中的 Codex 语义 Executor 未原子化
  ↓
语义 Atom 被赋予结构/自报式 verification_mode
  ↓
Coverage 只路由 Main G0，未绑定 Lab Oracle 与 Certificate Check
  ↓
实现以 recipe-NN 和 Genome 元组制造结构唯一
  ↓
Producer 自报 coverage=1.0 / hard_gates_pass=true / duplicates=0
  ↓
Certification Matrix 重复检查相同弱不变量
  ↓
Linkage、C4、Certificate、Install 正确锁定同一个弱验证制品
  ↓
形式闭环为绿，但真实 Prompt 仍高度同质
```
...
| 层级 | 稳定 ID | 结论 | v2.9 控制 |
|---|---|---|---|
| 直接原因 | `DETERMINISTIC_TEMPLATE_SUBSTITUTED_FOR_REQUIRED_SEMANTIC_EXECUTOR` | 产品实现没有目标中声明的真实 Codex 语义调用路径 | Atom 化 Executor Role，并要求 Runtime Process Evidence |
| 直接原因 | `DIVERSITY_PROXY_INSUFFICIENT` | ID/Genome 元组/recipe 唯一被当作全概念差异 | 从原始 Prompt 独立重算，多指标 + 负向夹具 |
| 直接原因 | `PRODUCER_ASSERTION_USED_AS_ACCEPTANCE_EVIDENCE` | coverage、hard gate、duplicate 由 Producer 自己声明 | Evidence 记录 provenance；自报值只作输入，不可闭环 |
| 系统根因 | `TARGET_SEMANTIC_EXECUTOR_UNATOMIZED` | Target 描述中的关键执行器要求未进入 Atom | Target-to-Atom Completeness Gate |
| 系统根因 | `SEMANTIC_REQUIREMENT_VERIFICATION_MODE_DOWNGRADED` | 一个 Atom 同时包含 count 与 semantics，却只有结构 Mode | 正交分类；必要时拆 Atom；闭环使用 `ALL_OF` |
| 系统根因 | `SEMANTIC_ATOM_LAB_ROUTE_ABSENT` | Coverage 证明 Main 路由，不证明独立履约 | Coverage Graph v2 强制 Lab/Oracle/Certificate 边 |
| 系统根因 | `NEGATIVE_CASE_DECLARED_WITHOUT_EXECUTABLE_ORACLE` | 负向文本未成为实际矩阵 | Oracle 必须声明 executable、hash、fixtures、threshold 和 expected finding |
| 系统根因 | `CERTIFICATE_CLAIM_NOT_BOUND_TO_ORACLE_RESULT` | Certificate 的语义 claim 只绑定矩阵 PASS，没有绑定原始重算 | Claim-to-Oracle Result 双向 Hash 绑定 |
| 放大条件 | `VALIDATOR_ASSUMPTION_CORRELATED` | Main 与 Matrix 虽进程分离，仍共享“元组唯一”假设 | 实现独立性声明、不同代码路径、对抗样本与盲测 |
| 放大条件 | `HISTORICAL_ACTIVE_POINTER_AMBIGUOUS` | v0.1/v0.2 清单并存易被误读 | current release index + historical 标识；不改写旧证据 |

### 4.3 Five Whys 结论

1. 为什么同质 Prompt 通过？因为 Validator 只要求结构唯一和 Producer hard gate。
2. 为什么 Validator 只要求这些？因为语义要求没有可执行独立 Oracle。
3. 为什么没有 Oracle？因为 Frozen IR 只保存 `verification_mode` 和路由边，关键 Executor/语义闭环未原子化。
4. 为什么 Freeze 没有阻止？因为 v2.8 的独立证据原则没有全部物化为 Factory 的兼容性与完整性硬门。
5. 为什么证书仍为绿？因为认证链正确验证了制品身份、安装来源和弱矩阵结果，却没有被要求对 Certificate 语义 Claim 重新计算原始内容。

最终系统根因不是“Hash、Linkage 或授权链失效”，而是：

> Requirement → Evidence → Oracle → Certificate Claim 之间缺少机器可执行的证据类型兼容性和独立重算闭环。
...
- 由 Factory 执行作为需求输入或目标资源提供的可执行 Payload；视频模型和 JSX 只是 V-SCDSL Profile 中的实例；
- 由 Factory 自动批准执行授权；
...
### 7.2 Requirement Atom v2.9 Target Profile 示例（非平台常量）

```json
{
  "atom_id": "REQ-VSCDSL-SEMANTIC-MULTILANE",
  "statement": "必须调用 Codex CLI 进行多路差异化语义推理",
  ...
  "required_executor": {
    "role": "product_semantic_executor",
    "provider": "codex_cli",
    "real_execution_required": true,
    "fake_execution_sufficient_for_certification": false
  },
  "runtime_topology_contract_id": "TOPOLOGY-CODEX-MULTILANE-V1",
  "forbidden_proxies": [
    "template_rotation",
    "duration_only_variation",
    "numbering_only_variation",
    "self_reported_pass",
    "fake_launcher_only"
  ],
  "evidence_obligation_ids": [
    "EVID-CODEX-INVOCATIONS",
    "EVID-PORTFOLIO-LINEAGE",
    "EVID-INDEPENDENT-DIVERSITY"
  ],
  "oracle_ids": [
    "ORACLE-DETERMINISTIC-DIVERSITY",
    "ORACLE-INDEPENDENT-SEMANTIC-REVIEW"
  ],
  "acceptance_logic": {
    "operator": "ALL_OF",
    "items": [
      "REAL_EXECUTOR_PROOF_PASS",
      "DETERMINISTIC_DIVERSITY_PASS",
...
上述 Codex、多路推理和 V-SCDSL ID 仅演示 Target Profile 如何使用通用字段。平台 Schema 不得枚举这些 Target 值，也不得为它们设置默认值。
...
```text
“输出恰好 30 条且全概念差异”
  → ATOM-COUNT: exactly 30
  → ATOM-SEMANTIC-DIVERSITY: holistically differentiated
```
...
每项行为或语义要求必须绑定至少一个 `evidence_obligation`。以下是 V-SCDSL Target Profile 示例，不是平台固定 Executor：

```json
{
  "evidence_obligation_id": "EVID-CODEX-INVOCATIONS",
  "atom_ids": ["REQ-VSCDSL-SEMANTIC-MULTILANE"],
  "producer_owner": "MAIN_HARNESS_BUILD",
  "validator_owner": "EXTERNAL_LAB",
  "evidence_type": "RUNTIME_SUBPROCESS_PROOF",
  "required_fields": [
    "program_id",
    "run_id",
    "stage_id",
  ...
  "input_output_lineage_required": true,
...
样例中的 `lane_id`、`model`、`reasoning_effort` 和 `token_usage` 是该 Target Profile 的扩展字段，不属于所有 Runtime Evidence 的平台必填项。平台 Core 只强制身份、Scope、时间、进程、输入输出、完整性、Issuer、Attestation 和 Redaction 等通用字段；Profile Extension 必须由冻结 Atom 推导。
...
以下 JSON 是 V-SCDSL 差异性 Oracle 的 Profile 示例。平台只规定 Oracle Contract 的形状、绑定和关闭规则，不规定 Prompt 字段、相似度算法或阈值：

```json
{
  "oracle_id": "ORACLE-DETERMINISTIC-DIVERSITY",
  "oracle_version": "1.0.0",
  "oracle_class": "DETERMINISTIC_RECOMPUTATION",
  "owner": "EXTERNAL_LAB",
  "executable": {
    "artifact_ref": "lab/oracles/diversity_oracle.py",
    "sha256": "<REAL_SHA256>",
    "runtime": "python3"
  },
  "input_contract": {
    "artifact_role": "raw_portfolio",
    "schema_id": "VSC-PORTFOLIO-V1",
    "content_field": "candidates[].prompt_bundle.model_prompt",
    "producer_metrics_ignored": [
      "novelty_report.whole_concept_duplicates",
      "candidates[].evaluation"
    ]
  },
  "metrics": [
    {
      "metric_id": "PAIRWISE_CHAR5_JACCARD",
      "aggregation": "mean_and_p95",
      "threshold_ref": "THRESHOLD-VSCDSL-DIVERSITY-V1"
    },
    {
      "metric_id": "COMMON_BLOCK_RATIO",
      "aggregation": "portfolio",
      "threshold_ref": "THRESHOLD-VSCDSL-DIVERSITY-V1"
    },
    {
      "metric_id": "GENOME_PROMPT_GROUNDING",
      "aggregation": "coverage",
      "threshold_ref": "THRESHOLD-VSCDSL-DIVERSITY-V1"
    }
  ],
  "producer_metric_trusted": false,
  "shared_core_implementation_allowed": false,
  "positive_fixture_ids": ["FIXTURE-SUMMER-TRAVEL"],
  "negative_fixture_ids": [
    "NEG-DURATION-ONLY",
    "NEG-NUMBERING-ONLY",
    "NEG-TEMPLATE-ROTATION",
    "NEG-SAME-STORY-REPHRASE"
  ],
  "tamper_fixture_ids": [
    "TAMPER-CALL-COUNT",
    "TAMPER-MODEL-BINDING",
    "TAMPER-KNOWLEDGE-TRACE"
  ],
  "unknown_policy": "FAIL_CLOSED_TO_HUMAN_ADJUDICATION",
  "adjudication_contract_id": "ADJUDICATION-VSCDSL-DIVERSITY-V1",
  "pass_logic": {
    "operator": "ALL_OF",
    "metric_threshold_set": "THRESHOLD-VSCDSL-DIVERSITY-V1",
    "required_fixture_pass_rate": 1.0
  },
  "result_schema_id": "ORACLE-RESULT-V1"
}
```

`<REAL_SHA256>` 是 Schema 示例中的显式待物化值；生成 Candidate 时不得保留该占位符。Oracle Contract 必须在 Freeze 前绑定真实文件、真实 Hash、阈值版本、算法版本、输入字段、Unknown 策略、预期 Finding 和结果 Schema。
...
2. Lab 在独立进程中运行；
3. Lab 从 Target Profile 声明的原始领域输出重新计算 Requirement-specific 指标，不采信 Producer 汇总值；V-SCDSL Profile 才具体化为从原始 Prompt 重算差异指标；
...
8. Lab 不得只修改 Producer 的内部表示后复用 Producer Validator 作为语义篡改测试；Genome 只是 V-SCDSL Profile 中的内部表示实例；
...
Runtime Topology 是目标 Harness 的 Requirement Profile，不是 Foundry 平台的全局固定拓扑。Foundry 只负责验证“声明的拓扑与真实 Receipt 是否一致”；不得要求所有项目都使用 Codex、6 Lane、8 次 baseline 或 10 次上限。

对 V-SCDSL 专属 Profile `codex-multilane-v1`，可由 Foundry 根据已冻结的目标 Atom 生成：

```json
{
  "topology_id": "TOPOLOGY-CODEX-MULTILANE-V1",
  "baseline": [
    {"stage": "intent", "count": 1},
    {"stage": "creative_lane", "count": 6, "concurrency_max": 3},
    {"stage": "portfolio_critic", "count": 1}
  ],
  "supplement": {
    "count_max": 2,
    "only_rejected_positions": true
  },
  "total_call_count_max": 10,
  "baseline_call_count": 8,
  "profile_scope": "PROGRAM-VSCDSL-VIDEO-PROMPT-HARNESS",
  "derived_from_atom_ids": [
    "REQ-VSCDSL-SEMANTIC-MULTILANE"
  ],
  "model_binding_scope": "ONE_HARNESS_RUN",
  "model_drift_policy": "FAIL_CLOSED",
...
- 所有调用使用同一冻结模型绑定；
- 子进程使用 `stdin=DEVNULL`；
- 每次调用有 Attempt-scoped 输出；
- 没有静默降级到模板生成。

如果冻结 Requirement 没有明确规定 Lane 数、并发数或 Supplement 上限，Factory 不得自行发明上述数字；应停在 Clarification/Classification 状态。对于不需要多路语义 Executor 的项目，Topology Contract 可以是单进程、非 Codex 或 `NOT_APPLICABLE`，但必须有 Atom 级依据。
...
允许这些值出现在带 `profile_scope` 的 Target Profile、案例说明、测试 Fixture 和 Canary 证据中。审计必须证明平台从冻结 Atom/Profile 读取参数，而非对 V-SCDSL、Prompt、Codex、Seedance、Genome 或固定 Lane 进行分支判断。
...
## 18. 跨领域 Golden Canary（`NON_NORMATIVE_CONFORMANCE_PROFILES`）

V-SCDSL 是根因发现案例和首个语义 Canary，但不能单独证明平台通用性。v2.9 Release 至少必须覆盖以下四类互斥 Profile：

| Canary Profile | 目的 | 必须证明 |
|---|---|---|
| `STRUCTURAL_ONLY` | 纯 Schema/文件/Hash 要求 | 不创建语义 Oracle，不要求真实 Runtime，不出现 V-SCDSL 专属字段 |
| `BEHAVIORAL_NON_LLM` | 单进程本地确定性 CLI 或服务行为 | Runtime Evidence 可绑定非 Codex Executor；Topology 不要求 Lane |
| `SEMANTIC_STRUCTURED_OUTPUT` | 结构化结果的领域规则一致性 | Independent Oracle 能处理非生成式领域 Artifact 与指标 |
| `VSCDSL_SEMANTIC_MULTILANE` | 首个复杂语义 Target Profile | 模板伪差异、执行器缺失和自报 PASS 被拒绝 |

平台通用性测试还必须：

1. 用两组不同 Target Profile 生成相同平台 Schema，证明差异只存在于 Profile 数据；
2. 将 Provider、Artifact 字段、数量、Topology 和阈值全部改名/改值后仍能编译和验证；
3. 对平台核心实现执行领域词扫描，禁止基于 `V-SCDSL`、`Prompt`、`Codex`、`Seedance`、`Genome` 或固定 Lane 的条件分支；
4. 证明 `NOT_APPLICABLE` 合法：未声明 Runtime/语义义务的 Atom 不生成多余门禁；
5. 证明 Target Profile 缺少 Atom 来源时 Fail-closed，不能靠平台默认值补齐。

### 18.1 V-SCDSL Target Profile 定位
...
E2E 至少分别运行 `STRUCTURAL_ONLY`、`BEHAVIORAL_NON_LLM`、`SEMANTIC_STRUCTURED_OUTPUT` 和 `VSCDSL_SEMANTIC_MULTILANE`，不得只用 V-SCDSL Fixture 验证平台。

还必须执行两类去偏置测试：

1. **Case Ablation**：完全移除 V-SCDSL Profile、正文案例与 Fixture 后，Core Schema、Gate、状态机、CLI、Extension Point 和通用路由的规范化 Hash 不变；
2. **Case Substitution**：换入与生成式视频无关的 Target Profile，只允许 Profile 数据、领域 Rule、工具绑定和 Oracle 改变，平台控制流不得随故事内容转向。
...
2. `BEHAVIORAL_NON_LLM`，证明 Runtime Evidence 不依赖 Codex 或多 Lane；
3. `SEMANTIC_STRUCTURED_OUTPUT`，证明 Oracle Contract 可处理非生成式领域 Artifact，不依赖 Prompt/Portfolio/Genome；
4. `VSCDSL_SEMANTIC_MULTILANE`，使用同一冻结需求重新生成 v2.9 Start Package，验证已知缺陷被系统主动拒绝。
...
9. `STRUCTURAL_ONLY`、`BEHAVIORAL_NON_LLM`、`SEMANTIC_STRUCTURED_OUTPUT` 和 `VSCDSL_SEMANTIC_MULTILANE` 四类 Canary 全部通过；
...
38. 至少一个非V-SCDSL Canary能根据输入自动提出Architecture候选、解释Stage/Subharness/Module选择并暴露缺失设计问题，而不是只生成验证合同。
...
### 22.2 V-SCDSL Target Profile 验收
...
| V-SCDSL 规则渗入平台 | `audit-platform-generality` + 四类跨领域 Canary + Profile 参数来源追溯 |
...
5. 将V-SCDSL只登记为Case Finding与Profile输入，不允许其参与Core Schema和控制流编译
...
20. 最后加载V-SCDSL Target Profile，运行隐性Rule提取、架构候选、生产行为和Oracle Canary
...
| V-SCDSL 是否仍是平台默认 | **不是**。只保留 Target Profile/Golden Canary 身份 |
...
5. **当前案例的入口就是假阳性**：V-SCDSL首先表现为弱Validator允许模板轮转通过，导致根因分析从Oracle倒推，未充分前推到“Producer为什么会生成模板”。
...
例如Controller规定：必须读取Input Facts和Applicable Rules，产生N个Concept Candidate，逐项调用Prompt Compiler，达到预算后停止；LLM只能为某个Candidate填入结构化概念，不得把N改成任意数量、跳过Rule Binding、增加未授权工具或把失败写成PASS。
...
| 类型 | 回答的问题 | 示例 |
|---|---|---|
| `DOMAIN_RULE` | 业务产出必须怎样形成 | 多图角色绑定不得交换 |
| `GOVERNANCE_POLICY` | 谁在什么条件下允许做什么 | 未授权不得联网或安装真实目标 |
| `HEURISTIC` | 通常怎样做效果更好 | 模糊主题优先从机制、叙事、场景和视听四个维度扩展 |
| `TARGET_CAPABILITY_CONSTRAINT` | 工具/模型客观能做什么 | 某Target只接受特定输入格式或上下文长度 |
| `ORACLE_RULE` | 如何判断结果满足要求 | 重新计算角色一致性和概念重复率 |

用户所说的`politic`在工程上建议统一写作`Policy`。V-SCDSL中“隐性Policy”实际上混合了Domain Rule、Heuristic、Target Capability和少量Governance Policy；如果不拆开，就会把创意建议错误做成硬门，或把安全边界错误当成可选建议。
...
## 39. V-SCDSL隐性规则显式化与生产架构候选（`NON_NORMATIVE_TARGET_PROFILE_NOTE`）
...
### 39.2 V-SCDSL Target Profile的候选生产拓扑
...
### 40.1 当前判断：作为回溯报告合理，作为平台规范偏重

R5回读中，`V-SCDSL`在正文出现37次，并同时承担七种角色：触发案例、现场证据、根因解释、Schema示例、Oracle示例、Golden Canary和候选生产拓扑。每一处都标记为Profile或示例，能够降低直接硬编码风险，但不能消除人和模型的锚定效应。

因此要区分本文的两种用途：

- 作为“为什么需要v2.9”的回溯报告，保留较完整案例有助于证明问题不是凭空假设；
- 作为“v2.9平台如何实现”的规范，当前案例权重过大，容易让首个案例决定术语、优先级、默认拓扑、测试结构和实现顺序。

实际v2.9新根不得直接把本文当成单一Normative Spec。必须把第3、18、22.2、24和39节拆到案例/Profile/Conformance目录；Core只保留抽象后的合同和晋升依据。

### 40.2 可能发生的三类偏移

1. **升级方向锚定**：因为首个失败来自生成式视频，平台不断强化语义多样性、模型调用、候选选择和多路拓扑，而低估确定性CLI、数据迁移、权限系统等领域；
2. **策略转向累积**：新案例不适配旧路径时，不修改或替换旧设计，而是增加一个特殊分支、降级策略、重试策略或新Subharness；
3. **链路只增不减**：每次修复都追加Gate、状态、CLI和人工门，旧路径仍保留，最终形成多个看似安全但彼此重叠、成本高且难恢复的控制链。

这类问题统称`CASE_DRIVEN_CONTROL_FLOW_ACCUMULATION`。它与领域词硬编码不同：即使代码中没有`V-SCDSL`字符串，也可能已经把该案例的思考顺序和异常路径固化为平台控制流。
...
```text
spec/core/
  domain-neutral schema, lifecycle, extension points, gates

profiles/<profile_id>/
  domain rules, capability bindings, tools, thresholds, oracles

cases/<case_id>/
  narrative, source evidence, fixtures, expected findings
```

唯一允许的依赖方向是：

```text
Case → Target Profile → Frozen Core Extension Point
```
...
```text
Core → Case
Core → 某个Profile的字段名或默认值
加载Profile → 动态增加Core状态或分支
Fixture失败 → 直接追加平台Stage/Subharness
```

这里的“物理分离”不仅是分目录。构建Core时不挂载`profiles/`和`cases/`也必须成功；静态依赖图、规范化Hash和Case Ablation共同证明没有反向依赖。

### 40.4 案例发现问题后先分类，不直接升级

每个案例Finding先进入以下五选一分类：

| 分类 | 含义 | 允许的修改位置 |
|---|---|---|
| `PLATFORM_INVARIANT_DEFECT` | 违反领域无关的平台不变量 | 满足晋升门后修改Core |
| `PROFILE_VARIATION` | 只是该领域的Rule、能力、阈值或工具不同 | 只修改Target Profile |
| `IMPLEMENTATION_DEFECT` | 现有合同正确，但实现没有按合同运行 | 修Producer/Controller/Module，不改架构 |
| `FIXTURE_OR_ORACLE_DEFECT` | 测试输入或判断方法错误 | 修Fixture/Oracle，不改生产链 |
| `NO_CHANGE_REQUIRED` | 预期差异、证据不足或不在范围内 | 记录并停止 |

只有`PLATFORM_INVARIANT_DEFECT`可以进入Case-to-Core Promotion。即使晋升，也应先尝试把问题放入已有Extension Point；不能因为“案例很重要”就自动获得新平台节点。

### 40.5 如何限制转向策略和链路净增长

`CONTROL_FLOW_DELTA`不使用一个容易被优化的总分，而是逐项展示真实变化：

| 差量 | 优先处理 |
|---|---|
| 只改变内容、阈值、Rule或工具 | Profile参数化，Core零变化 |
| 旧节点职责错误 | 替换或收缩旧节点，不保留双路径 |
| 已有Extension Point可以承载 | 复用，不新增Stage/Subharness |
| 确有新的状态所有权、局部循环或失败隔离需求 | Architecture Reopen后考虑净新增 |

每个净新增节点必须同时回答：

1. 为什么参数化现有节点不能解决；
2. 为什么替换旧节点不能解决；
3. 新节点拥有哪个此前不存在的状态或责任；
4. 是否增加人工门、预算、恢复点、共同失效或外部依赖；
5. 哪条旧路径被删除，若不删除，为什么两条路径不是重复策略；
6. 删除该案例后，新节点是否仍被至少一个非来源Profile需要。

答不完整时，默认结果不是“再增加一个校验”，也不是“先留一条备用路径”，而是`ARCHITECTURE_CHANGE_NOT_JUSTIFIED`。

### 40.6 用V-SCDSL举例，但不让例子决定平台

| 案例观察 | 正确落点 | 不允许的升级 |
|---|---|---|
| 多图角色不能交换 | V-SCDSL `DOMAIN_RULE`与对应Oracle | 为全平台增加“多图身份Stage” |
| 36候选择30 | Profile中的Run Strategy参数 | 平台固定候选数和选择链 |
| 隐性规则藏在Prompt里 | 可晋升为通用“Rule必须显式登记”的合同候选 | 把V-SCDSL语法直接写入Core |
| 模型自由决定循环导致漂移 | 若非生成式Profile也复现，可晋升为确定性控制流不变量 | 为每种模型分别增加回退链 |
| 精确音频交给Sidecar | Target Capability与Compilation Rule | 平台要求所有项目都有Sidecar工程 |

重点不是禁止从案例学习，而是把学习结果先抽象、分类、跨领域反证，再决定落在Core、Profile、实现还是测试层。

### 40.7 R6的实际收敛决议

R6不继续为这个问题增加新的工程、CLI或独立运行阶段，而是：

- 扩展已有`audit-platform-generality`，输出Case Influence、Promotion和Ablation；
- 扩展已有`audit-architecture-run-traceability`，输出Control Flow Delta；
- 将案例章节明确降级为`NON_NORMATIVE`；
- 调整实施顺序：先在无案例条件下完成Core，再加载非来源Profile，最后才加载V-SCDSL；
- 将“参数化 → 替换 → 复用 → 净新增”写入Architecture Lock决策顺序。

因此，案例仍保留为根因证据和回归样本，但失去改变平台控制流的直接权力。这样既不会忘记真实失败，也不会让v2.9沿着一个案例不断转向和加长链路。
